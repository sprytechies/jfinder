...
class MultiModelForm extends CWidget
{
   ...
   /**
    ...
    public function init()
    {
		...
		$this->rowWrapper = array('tag' => 'div', 'htmlOptions' => array('class' => 'rss ' . self::CLASSPREFIX . 'row'));
		}
        ...
        if ($this->isCopyTemplate)
        {
            ...
            if ($this->parentWidget->hideCopyTemplate){
                if(Yii::app()->user->isGuest){
                    $htmlOptions['style'] = !empty($htmlOptions['style']) ? $htmlOptions['style'] . 'display:block;' : 'display:block;';
                }
                else{
