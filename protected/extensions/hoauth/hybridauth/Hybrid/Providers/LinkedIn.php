...
{ 
	...
	*/
	function getUserProfile()
	{
		try{
			...
			$response = $this->api->profile('~:(id,first-name,positions,educations,last-name,public-profile-url,picture-url,email-address,date-of-birth,phone-numbers,summary)');
		}
		...
		if( isset( $response['success'] ) && $response['success'] === TRUE ){
			...
			$this->user->profile->description = (string) $data->{'summary'};
                        if($data->{'educations'}){ 
                                ...
                                $this->user->profile->educations = $this->xml2array($data->{'educations'});
                                
                        }
                        if($data->{'positions'}){ 
                              $this->user->profile->positions = $this->xml2array($data->{'positions'});
                        }
	...
	*/
	function getUserActivity( $stream )
	{
        ...
        {
            $array = json_decode( json_encode($xmlObject) , 1);
            return $array;
        }
}
